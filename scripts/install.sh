...
NEXT_STEP_HELPER_SOURCE_FILE="$ROOT/scripts/workflow-next-step.mjs"
REPORT_HELPER_SOURCE_FILE="$ROOT/scripts/report-from-round2.mjs"
...
[ -f "$NEXT_STEP_HELPER_SOURCE_FILE" ] || die "Could not find workflow-next-step.mjs."
[ -f "$REPORT_HELPER_SOURCE_FILE" ] || die "Could not find report-from-round2.mjs."
...
copy_if_needed "$NEXT_STEP_HELPER_SOURCE_FILE" "$SKILL_DIR/scripts/workflow-next-step.mjs"
copy_if_needed "$REPORT_HELPER_SOURCE_FILE" "$SKILL_DIR/scripts/report-from-round2.mjs"
sync_dir_if_needed "$REFERENCES_SOURCE_DIR" "$SKILL_DIR/references"
sync_dir_if_needed "$ASSETS_SOURCE_DIR" "$SKILL_DIR/assets"
chmod +x "$SKILL_DIR/scripts/install.sh" "$SKILL_DIR/scripts/doctor.sh" "$SKILL_DIR/scripts/init-config.mjs" "$SKILL_DIR/scripts/round2-prepare.mjs" "$SKILL_DIR/scripts/log-usage.mjs" "$SKILL_DIR/scripts/workflow-next-step.mjs" "$SKILL_DIR/scripts/report-from-round2.mjs"
